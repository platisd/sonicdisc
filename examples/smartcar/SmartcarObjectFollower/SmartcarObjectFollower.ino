...
void loop() {
  ...
  if (newFilteredMeasurements) {
    ...
    // Now that the measurements are filtered, let's determine where to turn
    if (filteredMeasurements[6] != 0 && filteredMeasurements[6] < distanceThreshold) {
      // If the obstacle is in the front, stop rotating
      car.stop();
    } else {
      int rotation = 0; // The side to rotate. Plus is clockwise.
      for (int i = 0; i < NUM_OF_SENSORS; i++) {
        if (filteredMeasurements[i] == 0) { // Ignore the measurement if it is 0 (error value)
          continue;
        } else if (filteredMeasurements[i] < distanceThreshold) { // There is an obstacle nearby
          if (i < 6 && i > 2) {
            rotation--;
          } else if (i > 6 || i <= 2) {
            rotation++;
